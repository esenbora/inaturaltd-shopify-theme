...
- **Secrets**: `blog-engine/.env` holds `OPENROUTER_API_KEY`, `SHOPIFY_ADMIN_TOKEN`, etc. It is gitignored. Never commit it. The Railway `blog-cron` service reads them from its own service variables; the fallback workflow reads them from GitHub Actions secrets.
- **`processed.json`** (blog-engine ledger) is gitignored and lives on the Railway volume at `/data/processed.json` (`PROCESSED_LEDGER_PATH`). Do not commit or hand-edit it. Deleting it makes the engine re-scrape and republish everything as fresh drafts.
